...
- You have [installed the operator](installation) with a wasm binary (`dataplane.wasmSourceURL` or similar)
- You use **Envoy Gateway** for this walkthrough (Istio/Cilium: see [provider guides](../operator/dataplane-ecds))
- Envoy Gateway **extensionManager** points at kubeWAF port **5005** ([setup](../operator/envoy-gateway#configure-envoy-gateway-extension-server))
...
Configure the EG Extension Server as described in the [Envoy Gateway guide](../operator/envoy-gateway), then restart Envoy Gateway.
...
- [Architecture diagrams](../concepts/architecture)  
- [Writing rules](../operator/writing-rules) · [CRS](../operator/using-crs)  
- Other providers: [Istio](../operator/istio) · [Cilium](../operator/cilium)  
- [WAF CRD reference](../reference/crds/waf)

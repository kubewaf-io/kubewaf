...
served by the operator (see [WAF engine](../engine) and [challenge](../challenge)).
...
Operator guides: [WAF engine](../engine) · [challenge](../challenge).
...
Full guide: [Envoy Gateway Integration](envoy-gateway).
...
Full guide: [Istio Integration](istio).
...
<Callout type="info" title="Cilium Envoy build">
Creating the CEC is always supported. Whether Wasm/ECDS filters run depends
on your Cilium Envoy feature set. See [Cilium Integration](cilium).
</Callout>
...
- [WAF engine](../engine)
- [Proof-of-Work challenge](../challenge)
- [Challenge Proxy-WASM (PoW)](../pow-proxy-wasm)
